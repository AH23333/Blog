...
graph TB
    subgraph 线性结构
        L1["#[C|线性表] 顺序表/链表"] --> L2["#[C|栈与队列] LIFO/FIFO"]
        L2 --> L3["#[C|串] KMP/模式匹配"]
    end

    subgraph 非线性结构
        N1["#[C|树] 二叉树/BST/AVL"] --> N2["#[C|图] DFS/BFS/最短路径"]
        N1 --> N3["#[C|堆] 优先队列/堆排序"]
        N2 --> N4["#[C|集合] 并查集"]
    end

    subgraph 算法技术
        A1["#[C|查找] 二分/B树/散列"] --> A2["#[C|排序] 快排/归并/堆排"]
        A2 --> A3["#[C|复杂度] 时空权衡"]
    end
...
graph TD
    subgraph 线性表存储结构
        A["#[C|顺序存储] 顺序表<br/>逻辑相邻=物理相邻<br/>随机存取 O(1)"] --> C["应用场景：<br/>频繁查找、尾插尾删<br/>数据量相对固定"]
        B["#[C|链式存储] 链表<br/>逻辑相邻≠物理相邻<br/>顺序存取 O(n)"] --> D["应用场景：<br/>频繁插入删除<br/>数据量动态变化"]
    end
...
graph TD
    subgraph 栈的存储实现
        A["#[C|顺序栈]<br/>数组+top指针<br/>top=-1 空栈<br/>top=MaxSize-1 满栈"] --> B["共享栈：<br/>两个栈共用数组<br/>top1=-1, top2=MaxSize<br/>top1+1==top2 满栈"]
        C["#[C|链栈]<br/>单链表实现<br/>top=头结点后第一个<br/>无栈满问题"] --> D["入栈=头插<br/>出栈=删除头结点后第一个"]
    end
```
...
graph TD
    subgraph 队列实现
        A["#[C|顺序队列]<br/>front=rear=0<br/>入队: rear++<br/>出队: front++"] --> B["问题: 假溢出<br/>front!=0但rear==MaxSize"]
        B --> C["#[C|循环队列]<br/>front=rear=0(队空)<br/>(rear+1)%MaxSize==front(队满)"]
        D["#[C|链队列]<br/>front→头结点<br/>rear→尾结点<br/>入队=尾插, 出队=头删"] --> E["双端队列:<br/>两端均可入队/出队<br/>输出受限/输入受限"]
    end
```
...
graph TD
    subgraph 深度优先遍历
        R["根=1"] --> L["左子树"]
        R --> R2["右子树"]
        PRE["#[C|先序] NLR: 1→左→右"] --> |"先访问根"| R
        IN["#[C|中序] LNR: 左→1→右"] --> |"中间访问根"| R
        POST["#[C|后序] LRN: 左→右→1"] --> |"最后访问根"| R
    end

    subgraph 广度优先遍历
        LEVEL["#[C|层序] 从上到下, 从左到右<br/>需借助队列实现"]
    end
```
...
graph TD
    subgraph 线索化
        A["普通二叉树: 大量空指针<br/>n个结点有n+1个空链域"] --> B["#[C|线索化]: 利用空链域<br/>ltag=0: lchild指向左孩子<br/>ltag=1: lchild指向直接前驱<br/>rtag=0: rchild指向右孩子<br/>rtag=1: rchild指向直接后继"]
    end

    subgraph 线索化类型
        C["先序线索化"] --> D["寻找后继容易"]
        E["中序线索化"] --> F["#[G|最常用]<br/>可方便找到前驱和后继"]
...
```mermaid
graph TD
    A["#[C|并查集] 处理不相交集合<br/>的合并与查询"] --> B["三种操作"]
...
graph TD
    subgraph 邻接矩阵
        M1["n×n的二维数组<br/>A[i][j]=1 有边<br/>A[i][j]=0 无边"] --> M2["优点: 判断边 O(1)<br/>缺点: #[R|空间 O(n²)]<br/>适合稠密图"]
    end
...
graph TD
    subgraph Dijkstra单源最短路径
        ...
        D4 --> D5{"所有顶点已访问?"}
        D5 -->|"否"| D2
        D5 -->|"是"| D6["#[G|完成!] 复杂度 O(n²)<br/>仅适用于非负权边"]
    end
...
graph TD
    ...
    subgraph 冲突处理
        C1["#[C|开放定址法]<br/>Hi=(H(key)+di) mod m"] --> D1["线性探测: di=1,2,3..."]
        C1 --> D2["平方探测: di=1²,-1²,2²,-2²..."]
        C1 --> D3["再散列: di=i×H2(key)"]
        C1 --> D4["伪随机: di=随机序列"]
        C2["#[C|链地址法]<br/>同义词链成链表"] --> D5["优点: 无堆积<br/>适合表长不确定"]
    end
```
...
graph TD
    subgraph 插入排序
        ...
        I2 --> I3["#[C|希尔排序]<br/>缩小增量排序<br/>O(n¹·³) ~ O(n²) / 不稳定"]
    end

    subgraph 交换排序
        E1["冒泡排序<br/>O(n²) / 稳定"] --> E2["#[C|快速排序]<br/>分治+枢轴划分<br/>O(n·log n) / 不稳定"]
    end

    subgraph 选择排序
        S1["简单选择排序<br/>O(n²) / 不稳定"] --> S2["#[C|堆排序]<br/>O(n·log n) / 不稳定<br/>建堆O(n), 排序O(n·log n)"]
    end

    subgraph 其他排序
        M1["#[C|归并排序]<br/>O(n·log n) / 稳定<br/>空间O(n)"] --> M2["基数排序<br/>O(d×(n+r)) / 稳定<br/>适合多关键字"]
    end
```
...
graph TD
    subgraph 堆的定义
        H1["#[C|大根堆]: 每个结点 ≥ 孩子<br/>用于升序排序"] --> H2["#[C|小根堆]: 每个结点 ≤ 孩子<br/>用于降序排序"]
    end
...
graph TD
    subgraph 阶段1: 数据存储
        A["原始数据: 学号+成绩<br/>多条记录"] --> B["用#[C|顺序表]存储<br/>支持随机访问"]
        B --> C["用#[C|链表]辅助<br/>动态插入删除"]
    end

    subgraph 阶段2: 数据组织
        C --> D["构建#[C|BST]便于查找<br/>学号为关键字"]
        D --> E["BST不平衡→转为#[C|AVL]<br/>旋转保持平衡"]
        E --> F["建立#[C|散列表]<br/>用除留余数法+链地址"]
    end

    subgraph 阶段3: 数据处理
        F --> G["用#[C|栈]实现<br/>递归/表达式求值"]
        F --> H["用#[C|队列]实现<br/>层序遍历/缓冲区"]
        F --> I["用#[C|图]表示<br/>课程依赖关系"]
    end
